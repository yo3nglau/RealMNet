...
class VisionFM(BaseBackbone):
    ...
    def __init__(self, img_size=[224], patch_size=16, in_chans=3, num_classes=0, embed_dim=768, depth=12,
                 ...
                 init_values=0, use_mean_pooling=False, masked_im_modeling=False, feat_concat=False, use_flash=False, init_cfg=None):
        super(VisionFM, self).__init__(init_cfg)
    ...
    #         x = x[:, 1:].mean(dim=1) if self.global_pool == 'avg' else x[:,
    #                                                                    0]  # default in Sp16: [B, 384], only take the cls_token
    ...
    def forward(self, x, return_all_tokens=None, mask=None, maps=None):
        ...
        if maps is not None and self.mask_ratio < 1:
            ...
            x = torch.gather(x, dim=1,
                             index=active_idx)  # default in Sp16: [B, 196, 384] -> [B, 148, 384] # select the subset by the given saliency map
        ...
        if return_all_tokens:
            return x  # all tokens but cls_token
        # print("**********", x[:, 0].shape, "**********")
