...
from sklearn.metrics import hamming_loss, average_precision_score, \
    roc_auc_score, coverage_error, label_ranking_loss, brier_score_loss
    ...
    def compute_metrics(self, results: List):
        ...
        for k, v in metrics.items():
            if self.average is None:
                result_metrics[k + '_class-wise'] = v.detach().cpu().tolist()
...
class AveragePrecision(BaseMetric):
    ...
    def compute_metrics(self, results: List):
        ...
        result_metrics = dict()
        if self.average is None:
            result_metrics['mAP_class-wise'] = ap
        else:
            result_metrics['mAP'] = ap
    ...
    def calculate(pred: Union[torch.Tensor, np.ndarray],
        ...
        ap = average_precision_score(target, pred, average=average)
...
class HammingLoss(BaseMetric):
    ...
    def compute_metrics(self, results: List):
        ...
        hl = self.calculate(pred, target)

        result_metrics = dict()

        result_metrics['HammingLoss'] = hl
...
class AUC(BaseMetric):
    ...
    def compute_metrics(self, results: List):
        ...
        auc = self.calculate(pred, target, self.average)

        result_metrics = dict()

        if self.average is None:
            result_metrics['AUC_class-wise'] = auc
        else:
    ...
    def calculate(pred: Union[torch.Tensor, np.ndarray],
        ...
        auc = roc_auc_score(target, pred, average=average)
...
class Coverage(BaseMetric):
    ...
    def compute_metrics(self, results: List):
        ...
        coverage = self.calculate(pred, target)

        result_metrics = dict()

        result_metrics['Coverage'] = coverage
    ...
    def calculate(pred: Union[torch.Tensor, np.ndarray],
        ...
        coverage = coverage_error(target, pred)

        return coverage


@METRICS.register_module()
class RankingLoss(BaseMetric):
    default_prefix: Optional[str] = 'multi-label'
    ...
    def compute_metrics(self, results: List):
        ...
        rl = self.calculate(pred, target)

        result_metrics = dict()

        result_metrics['RankingLoss'] = rl
    ...
    def calculate(pred: Union[torch.Tensor, np.ndarray],
        ...
        rl = label_ranking_loss(target, pred)

        return rl


@METRICS.register_module()
class BrierLoss(BaseMetric):
    default_prefix: Optional[str] = 'multi-label'
    ...
    def compute_metrics(self, results: List):
        ...
        bl = self.calculate(pred, target)

        result_metrics = dict()

        result_metrics['BrierLoss'] = bl.detach().cpu().tolist()
    ...
    def calculate(pred: Union[torch.Tensor, np.ndarray],
        ...
        bl = pred.new_zeros(num_classes)
        for k in range(num_classes):
            bl[k] = brier_score_loss(target[:, k], pred[:, k])
