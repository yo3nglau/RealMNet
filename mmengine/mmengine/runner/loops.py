...
@LOOPS.register_module()
class BootstrapTestLoop(BaseLoop):
    """Loop for bootstrap test.
    ...
    Args:
        ...
        fp16 (bool): Whether to enable fp16 testing. Defaults to
            False.
        bootstrap_dataloaders (Dataloader or dict): A dataloader object or a dict to
            build a dataloader.
    """
    ...
    def __init__(self,
                 ...
                 dataloader: Union[DataLoader, Dict],
                 evaluator: Union[Evaluator, Dict, List],
                 bootstrap_dataloaders: Union[DataLoader, Dict],
                 fp16: bool = False):
        super().__init__(runner, dataloader)

        self.bootstrap_dataloaders = bootstrap_dataloaders
    ...
    def run(self) -> dict:
        ...
        test_metrics = self.evaluator.evaluate(len(self.dataloader.dataset))
        metrics_dict.update({'test_metrics': test_metrics})
        self.runner.call_hook('after_test_epoch', metrics=test_metrics)

        for bootstrap_idx, bootstrap_dataloader_dict in self.bootstrap_dataloaders.items():
            bootstrap_dataloader = self.runner.build_dataloader(
                bootstrap_dataloader_dict, seed=self.runner.seed, diff_rank_seed=False)
            for idx, data_batch in enumerate(bootstrap_dataloader):
                self.run_iter(idx, data_batch)

            # compute boostrap test metrics
            bootstrap_metrics = self.evaluator.evaluate(len(bootstrap_dataloader.dataset))
            metrics_dict.update({bootstrap_idx: bootstrap_metrics})
            self.runner.logger.info(f'{bootstrap_idx} finished')
            self.runner.call_hook('after_test_epoch', metrics=bootstrap_metrics)
